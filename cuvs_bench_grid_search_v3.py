...
def evaluate_parameter_combination_multithreading(params, vectors, queries, vectors_fp, queries_fp,
    ...
    try:
        ...
        def non_stop_search(cagra_index, search_params, query_batch, run_time, filter_obj):
            """Process queries continuously for run_time seconds."""
            query_batch_processed = 0
            query_times = []
            
            start_time = time.time()
            query_batch_gpu = cp.asarray(query_batch)  # Allocate once
            ...
            while time.time() - start_time < run_time:
                ...
                query_batch_processed += 1
            
            actual_runtime = time.time() - start_time
            return query_batch_processed, query_times
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            for query_batch in query_batches:
                futures.append(executor.submit(
                    non_stop_search, cagra_index, search_params, query_batch, run_time, filter_obj
                ))
        ...
        for future in futures:
            try:
                ...
                query_times.extend(result[1])
                query_batch_processed += result[0]
        ...
        qps = (query_batch_processed * batch_size) / (t2-t1)
        print("qps " + str(qps))
        
        latency = np.percentile(query_times, 99)
...
def evaluate_hnsw_combination_forloop(params, vectors, queries, vectors_fp, queries_fp,
    ...
    try:
        ...
        all_indices, total_search_time = batch_search_hnsw(queries, hnsw_index, search_params, filter_obj, k=k, batch_size=256)
        ...
        def non_stop_search(hnsw_index, search_params, query_batch, run_time):
            """Process queries continuously for run_time seconds."""
            query_batch_processed = 0
            query_times = []
            ...
            while time.time() - start_time < run_time:
                t1 = time.time()
                distances, indices =  hnsw_index.search(query_batch, k=k,params=search_params)
                query_time = time.time() - t1
                query_times.append(query_time)
                query_batch_processed += 1
        ...
        with ThreadPoolExecutor(max_workers=num_workers) as executor:
            for query_batch in query_batches:
                futures.append(executor.submit(
                    non_stop_search, hnsw_index, search_params, query_batch, run_time
                ))
